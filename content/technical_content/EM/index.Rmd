...
```{r derivatives, echo=F, eval=F}
incomplete_expr <- quote(4*log(mu1 * p1 + (1 - mu1) * p2) + 6*log(mu1 * (1-p1) + (1 - mu1) * (1-p2)))
incomplete_expr <- quote(log(mu1 *p1 + (1 - mu1) * p2))

incomplete_gauss <- quote(-0.5 * log(2 * pi * sigma^2) - 0.5 * ((x - mu) / sigma)^2)

D(expr = responsibility, name = 'mu')
D(expr = incomplete_expr_reduced, name = 'mu1')
D(expr = incomplete_expr, name = 'p1')
D(expr = incomplete_expr, name = 'p2')
...
```{python lower-equal-incomplete, echo=T, eval=F, results = 'hide', hlopts=list(language = 'python'), tidy=F}
import numpy as np
import pandas as pd
...
```{python lower-equal-incomplete-result, echo=F, eval=T,  class_name = 'python-code', results = 'hold', hlopts=list( language = 'python')}
import numpy as np
import pandas as pd
from scipy.stats import binom
...
```{r wine-years, echo=F, eval=F}
wine_preferences <- data.frame('year' = factor(x = c(rep(1, times = 3), rep(2, times =3)), levels = c(1, 2)),
                         'wine_type'= factor(x = c('Red', 'White', 'Sparkling', 
                                                   'Red', 'White', 'Sparkling'),
                                             levels = c('Red', 'White', 'Sparkling')), 
                         'percentage' = c(.20, .40, .40, 
                                          .40, .20, .40))
...
wine_preferences$year_label <- factor(wine_year1$year,
                                levels = c("1", "2"),
                                labels = c("P(x) = Wine Preferences in Year 1", "Q(x) = Wine Preferences in Year 2"))

    

wine_plot <- ggplot(data = wine_preferences, mapping = aes(x = wine_type, y = percentage, 
                                        fill = wine_type, 
                                        color = wine_type, 
                                        label = sprintf("%.2f", percentage))) + 
  geom_bar(stat = 'identity') + 
  geom_bar(stat = 'identity') + 
  scale_fill_manual(values = c('#8C1734', 'white', '#FCF4B3'))+
   scale_color_manual(values = c('#8C1734', 'black', '#FCF4B3')) + 
  scale_y_continuous(name = 'Proportion', expand = c(0, 0), limits = c(0, .70), 
                     breaks = seq(from = 0, to = .7, by = .1)) + 
  geom_text(nudge_y = .03, color = 'black', size = 8) + 
  scale_x_discrete(name = 'Wine Type') + 
  facet_wrap(~ year_label, nrow = 1) + 
  theme_classic(base_family = 'Helvetica', base_size = 24) + 
  guides(fill = F, color=F) + 
  theme( 
    strip.text = element_text(family = 'Helvetica', face = 'bold', size = 24),
...
ggsave(filename = 'images/wine_plot.png', plot = wine_plot, width = 16, height = 8)
```
...
def e_step(data, mu, p):
  ...
  assert len(mu) == len(p), "Number of estimates in mu is equal to the number of sucsess probabilities"
  assert np.round(np.sum(mu), 2) == 1, "Sum of mu should be equal to 1"
...
#M step 
est_old = m_step(responsibilities = rs)

#difference between lower bound and incomplete-data log-likelihood after M step 
compute_lower_bound(responsibilities=rs_old, mu = mu_fixed, p = est_old['p_new']) #-67.55981029970859
compute_incomplete_log_like(data=binom_mixture_data, mu = mu_fixed,  p = est_old['p_new']) #-61.98106362224311

#amount that lower bound increased by after M step (difference in expected complete-data log-likelihoods. )
rs_old.apply(compute_expected_complete_like, mu = mu_fixed, p = p, axis=1).sum()  - rs_old.apply(compute_expected_complete_like, mu = mu_fixed, p = es_old['p_new'], axis=1).sum()


#difference between lower bound and incomplete-data log-likelihood = KL divergence 
#between entropy and cross-entropy
cross_entropy = -np.sum(resp_colummns_old.values * np.log(resp_colummns_new.values))
...
#create dataframe with all possible probability combinations [0.7, x]
incomplete_data_like = pd.DataFrame({'p1': np.arange(start = 0, stop = 1, step = 0.01)})
incomplete_data_like.insert(0, 'p2', 0.1)  

incomplete_data_like['likelihood'] = incomplete_data_like.apply(lambda row: compute_incomplete_log_like(data=binom_mixture_data, 
mu=mu_fixed, p=[row['p1'], row['p2']]), axis=1)


#compute full lower bound 
rs_old = e_step(data = binom_mixture_data, mu = mu_fixed, p = p)

lower_bound_df = pd.DataFrame({'p1': np.arange(start = 0.01, stop = 1, step = 0.01)})
lower_bound_df.insert(0, 'p2', 0.1)  

lower_bound_df['likelihood'] = lower_bound_df.apply(lambda row: compute_lower_bound(responsibilities=rs_old, 
mu=mu_fixed, p=[row['p1'], row['p2']]), axis=1)
...
#second lower bound 
est_old = m_step(responsibilities = rs_old)
est_old['p_new'][1] = 0.1
rs_new = e_step(data = binom_mixture_data, mu = mu_fixed, p = est_old['p_new'])


lower_bound_df_2 = pd.DataFrame({'p1': np.arange(start = 0.01, stop = 1, step = 0.01)})
lower_bound_df_2.insert(0, 'p2', 0.1)  

lower_bound_df_2['likelihood'] = lower_bound_df_2.apply(lambda row: compute_lower_bound(responsibilities=rs_new, 
mu=mu_fixed, p=[row['p1'], row['p2']]), axis=1)


```

```{r em-plot, echo=F, eval=F}
