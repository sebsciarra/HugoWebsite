...
```{r r-code, tag = 'R',  echo=T, eval=F, hlopts=list(hl_lines= paste0('[', 1,',', "2", ",", "5", ']'))}
...
```{python, echo=T,  eval=F, hlopts=list(hl_lines= paste0('[', 2,',',3, ',', 4,']'), language = 'python'), engine.path = '/usr/bin/python3', tidy=F}
...
```{r java-code, engine = 'javascript', echo=T, eval=F, hlopts = list(language = 'java')}
...
```{r css-code, engine = 'css', echo=T, eval=F, hlopts = list(language = 'css')}
...
```{hmtl html-code, echo=T, eval=F, hlopts = list(language = 'html')}
...
```{r parameterValues1, echo=F, eval=F}
